...
describe('selectForMoment', () => {
  ...
  it('never chooses an artist that just played', () => {
    const pool = [candidate(song('a track', 'Harbour')), candidate(song('another', 'Low Antenna'))]
  ...
  it('counts a collaboration credit as the artist that just played', () => {
    ...
    const picked = selectForMoment(pool, { ...AFTERNOON, avoidArtists: ['Harbour'] }).songs
  ...
  it('with nothing to match on, falls back to newest first — today\'s order', () => {
    const pool = [
    ...
    const picked = selectForMoment(pool, AFTERNOON).songs
    expect(picked.map((i) => i.title)).toEqual(['newer', 'older'])
  })
