...
import { cliConversation, cliPermission, lineReader, musicSetupCheck, runMusicSetup } from '../src/guide.ts'
...
describe('runMusicSetup (spec 03-03 §3 flow)', () => {
  ...
  it('a failed preflight on a closed stdin skips instead of blocking startup', async () => {
    // codex-review regression: `< /dev/null` (or any service run) must still
    // degrade to talk-only — a startup check never wedges the radio.
    const { host, infos } = fakeHost([], { atEof: true })
    const { guide, requests } = fakeGuide()
