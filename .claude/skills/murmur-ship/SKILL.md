...
3. **Test gate.** `npm test` (vitest) all green, plus `npm run typecheck` and `npm run lint`. Max 3 fix→retry rounds; still red → **Paused** with the failing output. A flaky / non-deterministic failure → Paused (do not loop it to green).
...
7. **Commit.** Group into logical commits via `smart-commit`. The org commit-message convention applies. Stage explicit paths (the main checkout is shared); the pre-commit gates (source-language + tsc + oxlint + path-governance) run on commit and need `node`/`npx` + `python3` on PATH.
