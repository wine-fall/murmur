...
6. **Closing review — prefer the other engine.** **After** the test gate is green, once per branch. Preferred reviewer is a *different* engine for independent eyes: if `codex` is installed and `codex login status` reports logged in, run **`codex exec review`** — the non-interactive path. `codex review` (no `exec`) is the interactive one: it exits 0 but prints only its exploration trace when stdout is redirected, so the findings and verdict are lost. Require a clean tree and try the latest, most capable Codex model available to the installed CLI and account. The command below pins the currently verified choice; when a newer model is available, replace both `model` and `review_model` with it. If that model is unavailable, fall back to the shown choice and record why. Scope the diff in the prompt and `tee` the output:
   ```bash
   set -o pipefail; test -z "$(git status --porcelain)" || { echo dirty; exit 1; }
   codex exec review -c model="gpt-6-astra" -c review_model="gpt-6-astra" -c model_reasoning_effort="xhigh" \
     "Your review scope is exactly the diff from \`git diff --find-renames --find-copies origin/main...HEAD\`; read it first and review nothing outside it. Review the trade-off, not only correctness. Establish the intended benefit, then adversarially test negative impact. At minimum consider performance, complexity, and monetary or operational cost, plus reliability, compatibility, security, privacy, and user-visible degradation when relevant. Scale depth and evidence to risk; separate facts, inferences, and open questions; do not invent findings. If there are no issues, say that clearly. End with 'Trade-off verdict: acceptable', 'Trade-off verdict: needs evidence', or 'Trade-off verdict: request changes', the decisive reason, and what evidence would change it." \
     2>&1 | tee <scratchpad>/peer-review.log | tail -5
   ```
   A run that ends without a `Trade-off verdict:` line did not review anything — check the invocation before believing it. Auth / quota / timeout = a **mechanical skip**, not "no findings" — say so. Fall back to the `code-review` skill only when codex is unavailable. Triage the findings with technical rigor — do not blindly implement: verify each against the code, fix the genuine ones (then re-run `pnpm test`), dismiss false positives with a one-line reason. One round. Record the outcome for the PR body: `Peer review (codex …): N findings, M applied, K dismissed` — or `Peer review: skipped — <reason>`.
