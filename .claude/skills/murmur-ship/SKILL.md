...
4. **Test gate.** `npm test` (vitest) all green, plus `npm run typecheck` and `npm run lint`. Max 3 fix→retry rounds; still red → **Paused** with the failing output. A flaky / non-deterministic failure → Paused (do not loop it to green).
