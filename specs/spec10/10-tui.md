...
Esc stays the interrupt. **State and action are drawn apart (2026-09-16,
user report):** an option with `action: true` is a button — `( refresh now )`,
no tick box, never ticked — and Space or Enter on it submits immediately,
the ticks plus that key. **A multi list closes on an `( apply )` row the
CLIENT synthesizes** (`listRows` in `dock.ts` — not on the wire, the engine
never sees it): the card had no submit to look at, only a grey
`enter applies` footer, so nobody could tell what counted as submitting. Its
note is the state of the card — `2 changes`, or `nothing changed — Enter
leaves` — and Enter anywhere still submits, the row only makes that visible.
The synthesized row is a real cursor stop, so the pick model's indices count
