...
> §3.1).
> **Invites retired 2026-08-07** (product positioning converged — the spec
> 08/09 retirement precedent: the sections stay, marked, so the decision is not
> re-litigated): murmur is a **radio** — it keeps company, it does not solicit
> interaction. The whole "turn to you / slide back" chain is **removed from the
> code**: the invite state machine (`awaitingReply`, the window, the slide-back
> cue), `TalkBeat.invite`, the `invite`/`slide-back` prompt cues,
> `invitesEnabled` / `--no-invites`, `ProgramState.awaitingReply`, and the pet's
> `turn` pose. The listener-initiated talkback path is untouched — typing to
> the radio is the interaction model. Anchors, activity pacing, and gating
> (deliverables 2-4) remain built and live. Sections below marked **[retired]**
> describe the removed degree and are kept as history only.
...
1. ~~**The proactive degree (model C).**~~ **[retired 2026-08-07]** A
   **Director-local** policy for when the radio turns to the listener and asks
   something, how it recognizes engagement, and **how it slides back** into the
   program when nobody answers — with no pressure and no repeated poking.
...
### 2.6 ~~Invite marking~~ — **[retired 2026-08-07]**
...
### 3.5 ~~Turning to you, and sliding back~~ — **[retired 2026-08-07]**
...
(`invitesEnabled` / `--no-invites` and the `INVITE_*` constants left with the
retired invite degree, 2026-08-07.)
...
- ~~An **invite** is a normal buffered beat that happens to be marked~~
  **[retired 2026-08-07]** with the invite degree.
- **Gating** pauses refills but never discards the buffer (§3.3).
- A **steer** still discards the buffer (spec 04 §3.3) — the listener took the
  floor.
...
12. ~~**Invite eligibility**~~ **[retired 2026-08-07]** with the invite degree.
13. ~~**Slide-back**~~ **[retired 2026-08-07]** with the invite degree.
14. ~~**Answered invite**~~ **[retired 2026-08-07]** — a typed line is ordinary
    talkback; there is no window to clear.
15. **Everything is switchable off**: `--no-anchors` / `--no-gating` restore
    exactly the pre-spec-07 behavior (the existing spec 01/03-02/04 suites pass
    unchanged with them set).
16. **Human acceptance (sensory, user-run — the real gate)**: over a real day,
    do the anchors land at the right moments, and does walking away make it go
    quiet without feeling dead? The agent produces the checklist; the user
    judges (master §11.2 layer 3).
