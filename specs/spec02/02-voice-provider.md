...
>   - **TS port (issue #54 Phase 2, 2026-07-27)**: the TypeScript implementation (now `src/`) ships **§3.6 (hosted HTTP) only** — `HostedVoice` with `MURMUR_TTS_*` config, sentence split + silence pad, seed pinning. The sidecar (§3.1–§3.2) and the local MLX backends (§3.3) are **not ported** (issue #54: local TTS deferred); acceptance §3 (sidecar kill/recover) and §4 (backend hot-swap) apply to the Python implementation only and retire at the Phase 5 cutover. §5.1/§5.2 (sounds human / warm) still gate on a by-ear pass.
...
- **Sentence pacing (split + silence pad):** fish TTS runs each sentence straight
  ...
  concatenates the wavs with a silence pad (default 0.8 s, by-ear tunable live via
