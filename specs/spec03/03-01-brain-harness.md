...
- `similar_music(artist: str, track?: str, limit?: int) -> {artists: [str]} | {tracks: [{title, artist}]} | {ok: false, error}` — **optional, key-gated**. Last.fm's public `artist.getsimilar` / `track.getsimilar`: what real listeners play alongside a seed. Present in the tool list only when `lastfmApiKey` is configured; a failed lookup returns `{ok: false, error}` (a lost turn, never a lost song — the task can still submit off `search_music` alone). Its purpose is the one thing prompt text cannot fix: with `search_music` as the only source, every candidate originates in the model's own recall, so the pick converges on the same few famous tracks forever.
...
7. **Candidates can come from outside the model.** With a key configured the task is offered `similar_music` and its results are real co-listening data; with none, the tool is absent and nothing else changes. A failed lookup costs a turn, not the song. (Unit-verifiable against a fake; the real API is a smoke.)
