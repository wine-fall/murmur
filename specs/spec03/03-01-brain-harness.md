...
> **Conventions**: English; written for a coding agent. Prompt text centralized in `src/prompts.ts`; no CJK in source (master §0).
...
Both sit behind the `ListeningData` contract, and the catalogue answering them is an **adapter** — `src/listening-data.ts` is to `ListeningData` what `src/music.ts` is to `MusicProvider`. Any source that can answer the same three questions (who is played alongside this artist, which tracks alongside this track, which of an artist's are played most) swaps in without touching the tools, the instruction, or the policy.
...
**The instruction is two halves** (`src/prompts.ts`):
