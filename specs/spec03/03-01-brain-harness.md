...
> **Extension (2026-08-25)**: the pick instruction is now **two halves** (§2.3): a code-owned CONTRACT and a listener-owned POLICY at `$MURMUR_HOME/music-policy.md`, re-read per pick (hot: an edit lands on the next song, no restart). The Director's anti-repeat depth went 8 -> 32.
> **Removal (2026-09-14)**: the key-gated `similar_music` / `top_tracks` tools and the `ListeningData` seam behind them are **gone**. They widened the candidate pool with a third party's aggregate play data — a stand-in, built before murmur could read the listener's own catalogues. [`../spec14/14-listening-taste.md`](../spec14/14-listening-taste.md) answers the same question from what this listener actually keeps, which is the better answer to the same failure, so the stand-in is deleted rather than kept beside it. The tools were key-gated and no key ever shipped, so nothing a listener had stops working.
...
- **`AudioClip.segment` — one slice of a longer source** *(added 2026-09-16, for spec 14 §2.9's "a chapter is a song")*. A ref may carry a **W3C media fragment** — `#t=<start>,<end>` in seconds, e.g. `https://www.youtube.com/watch?v=kx22t0PBrKM#t=612,868` — which is how a curated-channel candidate says "this chapter of that upload". `resolve` **strips and parses** it: yt-dlp is handed the bare url (it resolves either, verified), and the clip comes back with `segment: { startS, endS }` and `durationS` = the **segment's** length, never the two-hour upload's. That last part is the whole point of putting the length there: the Director's coda timing, the announce and the talk-ahead all read `durationS` and none of them has to learn what a segment is. `segment` is **additive and optional** — a talk clip and a whole track carry none, and every argument list built for them is byte-identical to what it was before. Where the segment IS read: the decoder (`decodeArgs` puts `-ss` before `-i` as it always did for the bed resume, and now a `-t <length>` after it — the **stop**, without which a chapter runs on into the rest of the playlist), the playability probe and the preview-trap probe (both open at `startS`, because the head of a long upload proves nothing about the slice half an hour in), and the preview trap's comparison (a segment clip is meant to be its **chapter's** length).
...
The tools handed to `run_task` for the music task — exactly two:
- `search_music(query: str, limit: int) -> {candidates: [TrackCandidate...]}` — wraps `MusicProvider.search`. It **executes** a search; it does not recommend. Which catalogues it may search is spec 14 §2.4's business: `youtube` always, `bilibili`/`netease` while the listener has them mounted, and `channels` — a local, network-free match over the recent uploads of the curated channel list (spec 14 §2.9) — while that pool holds something. `channels` is a place to **look**, never a statement about the listener's taste.
...
- `DEFAULT_MUSIC_POLICY` — the **taste** half, replaceable **wholesale** by `$MURMUR_HOME/music-policy.md`. Written as an ordered **playbook**, not a ban list: read the room, name the frame before the song, do not choose out of memory (reach from the frame — the scene, the year, the label, what the listener keeps), reach past the one song of theirs everybody knows, search for the specific thing, judge the candidates (official audio, no hour-long loops, vocals), stay in the listener's language and off the recently-played list. A ban list cannot say what to do *instead*, and the failure being prevented is not a bad song — it is the same handful of obvious ones forever. The policy may only name tools the task is actually offered; a unit test locks that, for the default and for the seeded template, which is written once and never rewritten.
...
7. **The pick task is offered exactly two tools**, `search_music` and `submit_pick`, and the policy halves name no tool beyond them — the default and the seeded template alike. (Unit-verifiable.)
