...
> **Extension (2026-08-25)**: the pick instruction is now **two halves** (§2.3): a code-owned CONTRACT and a listener-owned POLICY at `$MURMUR_HOME/music-policy.md`, re-read per pick (hot: an edit lands on the next song, no restart). A third, optional tool `similar_music` reads Last.fm's public similar-artist/similar-track data so candidates are not bounded by the model's own recall — key-gated, absent without one. The Director's anti-repeat depth went 8 -> 32.
...
Both key-gated tools are present only when `lastfmApiKey` is configured, and a failed lookup returns `{ok: false, error}` — a lost turn, never a lost song, since the task can still submit off `search_music` alone.
...
- **External**: `claude-agent-sdk` (in-process tool / MCP + agentic loop); **optional** Last.fm public API behind `similar_music` (a free key in `MURMUR_LASTFM_API_KEY`, no listening account; absent = no tool, no degradation elsewhere); `yt-dlp` (default source) — an **unbound external binary** per master §10.1's binary/library split (no pip extra; provisioned by the startup checks + setup guide, versioned independently of murmur). **Optional**: `musicdl` (user-installed provider).
