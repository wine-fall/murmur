...
> **TS port (issue #54 Phase 2, 2026-07-27)**: ported to `ts/` — the harness, the yt-dlp provider, the music tools (with the pull-time probe seam), context insertion, and `MusicProgrammer.nextTrack`, all re-derived for TS: the §2.1 `BrainTool` Protocol and its `terminal` flag are **not** transliterated. A task's tool set is a list of SDK `tool()`s built around a `finish` callback (`Harness.runTask<T>` in `ts/src/contracts.ts`), so "terminal tool with `ok == true`" becomes "the tool that calls `finish` ends the task with a typed value" — one seam, no wire-shape re-parsing. As in Python, the Director does not consume it yet (playback/scheduling land with the Phase 3 audio engine).
