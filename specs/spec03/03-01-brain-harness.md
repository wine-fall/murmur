...
> **TS port (issue #54 Phase 2, 2026-07-27)**: ported to TS — the harness, the yt-dlp provider, the music tools (with the pull-time probe seam), context insertion, and `MusicProgrammer.nextTrack`, all re-derived for TS: the §2.1 `BrainTool` Protocol and its `terminal` flag are **not** transliterated. A task's tool set is a list of SDK `tool()`s built around a `finish` callback (`Harness.runTask<T>` in `src/contracts.ts`), so "terminal tool with `ok == true`" becomes "the tool that calls `finish` ends the task with a typed value" — one seam, no wire-shape re-parsing. As in Python, the Director does not consume it yet (playback/scheduling land with the Phase 3 audio engine).
> **Extension (2026-08-25)**: the pick instruction is now **two halves** (§2.3): a code-owned CONTRACT and a listener-owned POLICY at `$MURMUR_HOME/music-policy.md`, re-read per pick (hot: an edit lands on the next song, no restart). Two more optional tools, `similar_music` and `top_tracks`, read a listening catalogue's public similar-artist / similar-track / top-track data so candidates are not bounded by the model's own recall — key-gated, absent without one. The Director's anti-repeat depth went 8 -> 32.
...
> **Conventions**: English; written for a coding agent. Prompt text centralized in `src/prompts/`; no CJK in source (master §0).
...
```
- **yt-dlp adapter (default):** `search` via `ytsearch{limit}:<query>` (metadata only, no download); `resolve` via `-f bestaudio/best --print "%(duration)s" --print urls` (audio-only preferred; falls back to a combined format where yt-dlp cannot offer bestaudio, e.g. JS-runtime-gated YouTube formats — ffmpeg decodes either) → a **stream URL** (`AudioClip.source` = URL, no disk download — master decision A) **plus the track's length** (`AudioClip.durationS`, the progress rail's denominator — spec 10 §3.3). `--print` in place of `-g` keeps it ONE extraction: measured at no cost over the bare `-g` (2.4s either way, 2026-08-25). The url is found by shape rather than by position — a duration handed to the decoder as a source is a song that dies silently — and yt-dlp's literal `NA` (a live stream, an extractor that omits it) reads as 0, the same "unknown" a missing `TrackCandidate.durationS` means. Covers YouTube + Bilibili.
...
The tools handed to `run_task` for the music task — two always, a third when a similar-music key is configured:
- `search_music(query: str, limit: int) -> {candidates: [TrackCandidate...]}` — wraps `MusicProvider.search`. It **executes** a search; it does not recommend.
- `similar_music(artist: str, track?: str, limit?: int) -> {artists: [str]} | {tracks: [{title, artist}]} | {ok: false, error}` — **optional, key-gated**. `artist.getsimilar` / `track.getsimilar` on the listening catalogue: what real listeners play alongside a seed. Its purpose is the one thing prompt text cannot fix: with `search_music` as the only source, every candidate originates in the model's own recall, so the pick converges on the same few famous tracks forever.
- `top_tracks(artist: str, limit?: int) -> {tracks: [str]} | {ok: false, error}` — **optional, key-gated**. `artist.gettoptracks`. Widening to a fresh artist and then airing their one famous single is the same habit one level down; play counts answer "which of theirs" where recall would answer with the canonical hit.

Both key-gated tools are present only when `listeningApiKey` is configured, and a failed lookup returns `{ok: false, error}` — a lost turn, never a lost song, since the task can still submit off `search_music` alone.

Both sit behind the `ListeningData` contract, and the catalogue answering them is an **adapter** — `src/music/listening-data.ts` is to `ListeningData` what `src/music/music.ts` is to `MusicProvider`. Any source that can answer the same three questions (who is played alongside this artist, which tracks alongside this track, which of an artist's are played most) swaps in without touching the tools, the instruction, or the policy.
...
**The instruction is two halves** (`src/prompts/`):
- `FIND_MUSIC_CONTRACT` — **code-owned**: use `search_music` to gather, judge, then end the task with `submit_pick`, which must carry `title`/`artist`/`announce`, and pick again on a resolve failure. A listener policy can never remove this, or a track would reach the air with no intro.
- `DEFAULT_MUSIC_POLICY` — the **taste** half, replaceable **wholesale** by `$MURMUR_HOME/music-policy.md`. Written as an ordered **playbook**, not a ban list: read the room, name the frame before the song, do not choose out of memory (widen with `similar_music`), then ask `top_tracks` which of theirs, search for the specific thing, judge the candidates (official audio, no hour-long loops, vocals), stay in the listener's language and off the recently-played list. A ban list cannot say what to do *instead*, and the failure being prevented is not a bad song — it is the same handful of obvious ones forever. The default names every widening tool that exists; a unit test locks that so the doc cannot drift behind the tool list.

`buildFindMusicInstruction(policy?)` composes the two. The policy file is read **fresh on every pick** (`src/music/music-policy.ts`) — no watcher, no cache, no reload command: a pick already costs seconds of network, so one small read is free. The unit of hotness is the **pick, not the song**: the Director primes a pick ahead of the boundary (spec 04 look-ahead), so an edit made after that priming reaches the song after it. Invalidating a primed pick on an edit is deliberately not done — it would throw away a paid discovery call to save one song of latency. Absent, empty, or unreadable = the built-in policy, so the radio plays exactly as it did before the file existed. HTML comments are stripped, so the seeded template can explain itself without its own instructions reaching the model. It is seeded (`wx`, never overwriting) the first time a run wires the music pipeline, because a policy the listener never sees is one they can never edit — and a machine that cannot play music at all has no picks to steer, so it gets no file.

A formal SDK *skill* remains out, now for a structural reason and not a preference: the music task runs with `settingSources: []` and `skills: []` (§2.1), so skill discovery is off by construction — enabling it would pull the user's entire local Claude Code configuration into the pick task, which is exactly what the isolation invariants forbid. The listener-owned policy file is murmur's own answer to the same need.
...
- **External**: `claude-agent-sdk` (in-process tool / MCP + agentic loop); **optional** listening catalogue behind `similar_music` / `top_tracks` — the public audioscrobbler protocol over HTTP, keyed by `MURMUR_LISTENING_API_KEY` and pointed by `MURMUR_LISTENING_URL` (default host Last.fm, whose key is free and needs no listening account; absent key = no tools, no degradation elsewhere); `yt-dlp` (default source) — an **unbound external binary** per master §10.1's binary/library split (no pip extra; provisioned by the startup checks + setup guide, versioned independently of murmur). **Optional**: `musicdl` (user-installed provider).
...
6. **The policy is the listener's, the contract is not.** A `music-policy.md` replaces the taste rules wholesale and reaches the very next pick with no restart; whatever it says, the pick still ends in a `submit_pick` carrying `title`/`artist`/`announce`. No file = byte-identical behavior to before the file existed. (Unit-verifiable.)
7. **Candidates can come from outside the model, at both levels.** With a key configured the task is offered `similar_music` (which artist) and `top_tracks` (which of theirs), both answering from real listening data; with none, both are absent and nothing else changes. A failed lookup costs a turn, not the song. (Unit-verifiable against a fake; the real API is a smoke.)
...
- **Settled (recorded so they are not re-asked):** selection heuristics live in the task instruction — its taste half, which the listener owns as a file (§2.3) — and **not** a formal SDK skill, which the harness's isolation invariants rule out by construction; a separate `MusicContext` (not an extension of the tool-less `ContextPack`); resolve latency is accepted as a small gap (real look-ahead is spec 04).
