...
- **Backlog adapters and their barriers** (all discussed; recorded so we don't revisit):
  ...
  - **Spotify**: playback needs **Premium at the protocol level** — librespot's own README states it will remain so, and a headless in-process stream is otherwise entirely feasible (cliamp does it with `go-librespot`; the "an app bound over AppleScript cannot honour a precise pick" reasoning that once stood here was wrong and is withdrawn 2026-09-06). **The user has no Premium and is not buying one** (decision, not a temporary state) → no Spotify playback. Spotify's Web API on a **free** account is a different matter: `/me/top/*` is the strongest taste signal of any platform, and spec 14 reads it.
...
- **Optional, user-installed gray providers (personal-experiment tier, §3.7 phase-1 — never a shipped default).** Behind the same `MusicProvider` seam a user may mount unofficial sources on their own machine, accepting the fragility/ToS risk: e.g. [`musicdl`](https://github.com/CharlesPikachu/musicdl) (a 50+-platform downloader — but it bundles a Node runtime and does **Widevine DRM circumvention**, a legal non-starter to *ship*), or Spotify via librespot (needs Premium). Because the ducking engine (spec 03-02) is **source-agnostic**, any of them works once it yields decodable audio.
  - **What spec 14 shipped, and which half of this line it moved.** The taste sources are all **opt-in** — nothing is read until the listener runs `/sources` — offered once as a consent card on the first run, which says so in those words — so "never a shipped default" stands untouched. What did move is "not in the shippable stack": NetEase and Soda Music ship *in* the stack as read-only taste sources. NetEase now reads through the plaintext `music.163.com/api` endpoints the platform's own web pages use (identity, kept lists, search — all ordinary GETs, no signing, no borrowed client key); Soda Music remains a reverse-engineered client and is the one genuinely gray thing shipped. Playback for both stays yt-dlp's.
  - **Auth reference — [`cliamp`](https://github.com/bjarneo/cliamp)** (Go/Bubble Tea terminal player): already implements the **login/auth flow** for these sources (checked 2026-09-06 against its `external/` tree: NetEase Cloud Music, YouTube/YouTube Music, SoundCloud, Mixcloud, Spotify, Tidal, Qobuz, Yandex, plus the self-hosted Navidrome/Plex/Jellyfin/Emby/Audiobookshelf/Lyrion — **not** Bilibili, and **not** Soda Music) via an interactive credential wizard. It is MIT-licensed (re-checked 2026-09-08); the taste-reading line that came of this is [`specs/spec14/14-listening-taste.md`](spec14/14-listening-taste.md). When an auth-requiring `MusicProvider` adapter is eventually built, reference **cliamp's auth mechanics** — how it obtains, stores, and refreshes per-service credentials/cookies. **Scope of the borrow**: we take the *auth flow only*, **not** its interaction model — cliamp is user-picks-and-logs-in (a player), whereas murmur is **AI-picks-by-context** (the brain selects the track; the user is a listener, not a selector). The credential/cookie plumbing is reusable; the song-selection UX is not.
...
| **14** | `listening-taste` ([`specs/spec14/14-listening-taste.md`](spec14/14-listening-taste.md)) | Five opt-in sources (YouTube, Bilibili, NetEase, Spotify, Soda) read into a deterministic taste digest the pick task and the pack receive; named-catalogue search (YouTube/Bilibili/NetEase); cookie-aware resolve; typed auth failure that says "expired" once; invitations as the one light nudge form. Read-only for Spotify/Soda by decision. | companion character | 03-01,05,10,12 |
