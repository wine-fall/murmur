...
- **No Chinese (CJK) anywhere in source** — comments, string literals, and docstrings alike (v1). The radio speaks Chinese only at runtime, produced by the model from the persona prompt; it is never a hardcoded string. Additionally, **comments are English-only** — ASCII plus a short allowlist of typographic symbols (dashes, ellipsis, curly quotes, `§`, `→`, the `①②③` memory-tier marks). Enforced by `scripts/check-source-language.ts` (wired via pre-commit; dependency-free, run straight off disk by node).
