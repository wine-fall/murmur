...
// The profile is always named: murmur reads one profile's cookie store, so
// the sign-in has to happen in that one. Chrome creates the profile directory
// when it opens a URL in one, so a profile that does not exist yet is no
// obstacle.
export function chromeOpenerFor(platform: NodeJS.Platform, url: string, profile: string): { command: string; args: string[] } {
  const pinned = `--profile-directory=${profile}`
  // `-n` is load-bearing: `open -a ... --args` hands its flags to Chrome only
  // when Chrome is not already running, so without it a listener with Chrome
  // open lands in whatever profile is in front of them. A second instance
  // relays the whole command line — profile included — to the running one
  // through Chrome's singleton, so the flag survives.
  if (platform === 'darwin') return { command: 'open', args: ['-na', 'Google Chrome', '--args', pinned, url] }
  // cmd re-parses its own line, so the profile name is quoted for the same
...
export function openInChrome(url: string, profile: string = chromeProfile()): void {
  spawnDetached(chromeOpenerFor(process.platform, url, profile))
}
