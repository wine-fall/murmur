...
// reading lines and delivers them here (src/director/director.ts).
...
import { escPulse, isYes } from '../setup/guide.ts'
import { ask, LineQueue, type Host } from '../host/host.ts'
import { REPORT_PROMPT, reportSystemPrompt } from '../prompts/report.ts'

export type ReportKind = 'bug' | 'feature'

// How long a draft the listener never sent stays around. The same fortnight the
// dev log keeps its days (src/support/dev-log.ts) — a report is evidence about a run,
...
export interface DeliverTools {
  ...
  // from whether a spawn seemed to work (src/support/deliver.ts explains why).
  hasBrowser(): boolean
...
export interface ReportDeps {
  ...
  // chose (src/support/dev-log.ts). Not a directory: an overridden log is one file, and
