...
// Exported because the bug report reads the same dated set (src/support/diagnostics.ts).
...
// (src/support/diagnostics.ts, src/support/sentinel.ts), so the two roads into a report cannot
